                        ...
                        const char *val_path)
{
    ...
    fprintf (f_com, "set terminal png size 1280, 1280\n");
...
{
    const char *palette = "set palette rgb 30,15,10";
   sprintf (path, "%s; plot '%s' using 1:2:(0.12*$3/(sqrt(($3)**2+($4)**2))):(0.12*$4/(sqrt(($3)**2+($4)**2))):(sqrt(($3)**2+($4)**2)) with vectors lw 2 lc palette notitle%c",
   // sprintf (path, "plot '%s' using 1:2:(10*$3):(10*$4) w vec lw 3 notitle%c",
    palette, fname, '\0');
}
...
void print_values (P_she p_s, double *V1, double *V2, FILE *fp)
{
    ...
    // x y x_delta y_delta
    for (int i = 0; i < p_s.M_y+1; i+=1)
    for (int j = 0; j < p_s.M_x+1; j+=1)
    {
        v1 = V1[i*(p_s.M_x+1) + j];
        v2 = V2[i*(p_s.M_x+1) + j];
        x_coord = j * p_s.h_x;//i % (p_s.M_x + 1) * p_s.h_x;
        y_coord = i * p_s.h_y;//(i / (p_s.M_x + 1)) * p_s.h_y;
        fprintf (fp, "%e %e %e %e\n", x_coord, y_coord,
